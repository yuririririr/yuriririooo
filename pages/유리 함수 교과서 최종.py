...
st.info("📘 이 교과서는 Streamlit으로 제작된 ‘유리함수 단원’ 학습 자료입니다.")
st.divider()
...
st.write("""
유리함수는 수학 교과서뿐 아니라 **우리 생활 속 다양한 현상**을 설명하는 데 사용됩니다.  
...
### 🚗 [2] 자동차 연비와 속도의 관계
자동차 속도를 너무 높이거나 낮추면 연비가 떨어집니다.  
연비 E는 속도 v에 대해 다음과 같은 형태로 표현됩니다.
\\[
E = \\frac{k}{(v - a)^2 + b}
\\]
→ 속도가 적정할 때 효율이 가장 높고, 너무 빠르거나 느리면 효율이 낮아집니다.
